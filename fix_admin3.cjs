...
// Try both LF and CRLF for export
const expIdx1 = a.indexOf('\nexport default Admin');
const expIdx2 = a.indexOf('\r\nexport default Admin');
const expIdx = Math.max(expIdx1, expIdx2);
if (expIdx < 0) {
  // Search for just 'export default Admin'
  const idx = a.indexOf('export default Admin');
  if (idx < 0) { console.log('ERROR: export not found'); process.exit(1); }
  // Find what precedes it
  const before = a.substring(Math.max(0, idx-30), idx);
  console.log('Found at idx', idx, 'before:', JSON.stringify(before));
  console.log('after:', JSON.stringify(a.substring(idx, idx+30)));
  process.exit(1);
}
console.log('export at:', expIdx, JSON.stringify(a.substring(expIdx, expIdx+30)));
...
console.log('AboutSection inserted');

// Insert popup at end of JSX
...
console.log('Done! showAbout:', a.includes('const [showAbout'), 'AboutSection:', a.includes('function AboutSection'));
